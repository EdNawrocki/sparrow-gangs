 ...
 */
 ...
 public class UnconstrainedTaskPlacer implements TaskPlacer {
   ...
   /** Number of threads that have been put to sleep for gang scheduling */
   ...
   public Map<InetSocketAddress, TEnqueueTaskReservationsRequest>
     ...
     // Get the full list of nodes and shuffle it.
     List<InetSocketAddress> nodeList = Lists.newArrayList(nodes);
     Collections.shuffle(nodeList);
     int totalNodes = nodeList.size();
 
     // Add every task's launch specification to unlaunchedTasks.
     ...
     // Distribute the total reservations evenly across all available nodes.
     for (int i = 0; i < totalNodes; i++) {
       int numReservations = reservationsToLaunch / totalNodes;
       if (reservationsToLaunch % totalNodes > i) {
         numReservations++;
       }
     ...
     LOG.debug("Request " + requestId + ": Launching enqueueReservation on " +
         totalNodes + " node monitors: " + debugString.toString());
     return requests;
   }
